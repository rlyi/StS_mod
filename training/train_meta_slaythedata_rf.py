"""training/train_meta_slaythedata_rf.py — обучение Random Forest из SlayTheData.

Аналог train_meta_slaythedata.py, но:
  - RandomForestClassifier вместо DecisionTreeClassifier
  - sample_weight: победные раны весят VICTORY_WEIGHT раз больше
  - Сохраняет модели в meta_*_slaythedata_rf.pkl
"""
import os
import sys
import json
...
VICTORY_WEIGHT    = 3.0   # победные раны весят в 3 раза больше
...
for name, X, y, w in models_to_train:
    ...
    path = os.path.join(MODELS_DIR, f'meta_{name}_slaythedata_rf.pkl')
