...
# 14 (player) + 7*8 (hand) + 4*5 (enemies) + 3*4 (potions) = 102
# Player [0-13]:  hp, energy, block, strength, dexterity, vulnerable, weak, poison,
#                 metallicize, corruption, barricade, draw_pile, discard_size, turn
# Hand   [14-69]: до 7 карт × 8: is_attack, is_skill, is_power, is_other, dmg/20, blk/20, cost/3, is_upgraded
# Enemies[70-89]: до 4 врагов × 5: hp_norm, intent_norm, block_norm, damage_norm, ritual_norm
# Potions[90-101]: 3 слота × 4: present, is_heal, is_attack, is_utility
OBS_SIZE    = 102
ACTION_SIZE = 51  # 7 cards × (no target + 4 enemies)=35 + end turn(35) + 3 potions × 5=15
POTION_SLOTS = 3  # слотов зелий в наблюдении
...
REWARD_KILL_ENEMY       =  1.0    # за каждое убийство
REWARD_DAMAGE_MULT      =  0.05   # атака 8 урона = +0.40 > блок (-0.05) — ломает стратегию блока
REWARD_LOSE             = -2.0    # смерть игрока
REWARD_DAMAGE_TAKEN_MULT= -0.03   # за единицу полученного урона
REWARD_TURN_PENALTY     = -0.05   # штраф за каждый ход (анти-столлинг)
