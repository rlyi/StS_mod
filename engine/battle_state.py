...
class BattleState:
    ...
    def __init__(self, player: Player, hand: List[Card] = None,
                 discard_pile: List[Card] = None, exhaust_pile: List[Card] = None,
                 draw_pile: List[Card] = None, monsters: List[Monster] = None, relics: dict = None,
                 ...
                 memory_by_card: dict = None, amount_scryed: int = 0,
                 saved_block_for_next_turn: int = 0, potions: list = None, amount_to_exhaust: int = 0):
        ...
        self.__is_first_play: bool = False
        self.__starting_energy: int = 0
        self.orbs: List[Tuple[OrbId, int]] = [] if orbs is None else orbs
        self.orb_slots: int = orb_slots
        self.memory_general: dict = {} if memory_general is None else memory_general
        self.memory_by_card: dict = {} if memory_by_card is None else memory_by_card
    ...
    def repeat_card(self, card: Card, target_index: int, reason_to_repeat,
    ...
    def resolve_card_play(self, card: Card, target_index: int):
    ...
    def transform_from_exhaust(self, card: Card, index: int):
    ...
    def spawn_in_hand(self, card_to_spawn: Card, amount: int = 1):
    ...
    def spawn_in_draw(self, card_to_spawn: Card, amount: int = 1):
        if self.player.powers.get(PowerId.MASTER_REALITY):
            card_to_spawn.upgrade = 1
    ...
    def spawn_in_discard(self, card_to_spawn: Card, amount: int = 1):
        if self.player.powers.get(PowerId.MASTER_REALITY):
            card_to_spawn.upgrade = 1
    ...
    def exhaust_card(self, card: Card, handle_remove: bool = True):
    ...
    def get_stance(self) -> StanceType:
        return self.get_memory_value(MemoryItem.STANCE)

    def retrieve_from_discard(self, retrieval_target: CardId, just_one: bool = True):
...
def is_card_playable(card: Card, player: Player, hand: list, draw_pile_count: int) -> bool:
    if card.cost == Cost.unplayable:
        return False
